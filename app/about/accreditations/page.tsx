...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/about/accreditations" },
};

const items: { name: string; what: string; why: string; verifyUrl?: string; logo?: string }[] = [
  { name: "HETAS", what: "UK industry body approving appliances, fuels and businesses in the solid fuel and biomass sector. Registered for Domestic and Non-Domestic systems (including 200kW to 1000kW).", why: "HETAS registration means our installations and servicing meet recognised standards, with paperwork that satisfies Building Control, insurers and RHI.", verifyUrl: site.hetasUrl, logo: "/hetas-logo.svg" },
              ...
              <article key={i.name} className="bg-[var(--white)] border-2 border-[var(--green)] p-6 md:p-8">
                ...
                {i.verifyUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener"
                    className="inline-block mt-4 text-sm font-semibold text-[var(--green)] underline underline-offset-4 hover:text-[var(--amber)]"
                  >
                    Verify on HETAS ↗
                  </a>
                )}
